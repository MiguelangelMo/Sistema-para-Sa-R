                  ...
                  <tbody v-if="arrayDetalle.length">
                    <tr v-for="(detalle,index) in arrayDetalle" :key="detalle.id">
                      ...
                      <td>
                        <span
                          style="color:red;"
                          v-show="detalle.descuento > (detalle.cantidad * detalle.precio)"
...
export default {
  data() {
    return {
      ...
      stock: 0
      /*greeter:{
        german: selecProveedor(search, loading) {
          let me = this;
          loading(true);
  ...
  methods: {
    ...
    pdfVenta(id){
      window.open('http://localhost:8000/venta/ventaPdf/'+id+','+'_black');
    },
    selecCliente(search, loading) {
      let me = this;
      loading(true);
    ...
    registrarVenta() {
      ...
      axios
        ...
        .then(function(response) {
          ...
          me.descuento = 0;
          me.arrayDetalle = [];
          window.open('http://localhost:8000/venta/ventaPdf/'+response.data.id+','+'_black');
        })
        .catch(function(error) {
          console.log(error);
        });
    ...
    validarVenta() {
      ...
      this.arrayDetalle.map(function(x) {
        if (x.cantidad > x.stock) {
          console.log(x);
          articulo = x.articulo + " con stock insuficinte";
          me.errorMostrarMsjVenta.push(articulo);
        }
      });
    ...
    desactivarVenta(id) {
      swal
        .fire({
          title: "Esta seguro de anular está VENTA?",
          type: "warning",
